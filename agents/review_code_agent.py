    ...
    def _generate_review_code_reply(
        ...
        while True:
            ...
            while True:
                ...
                - Whether the code you want to review exists in the history message? What is the code detail?
            ...
            if (source_res['source']['answer'] == 'github'):
                ...
                flag, code = self.generate_code_execution_reply(messages=_messages, config=self._code_execution_config)
                _messages.append({"role": "user", "content": code, "name": 'exec_result'})
                # 检查代码是否有误
                data = {}
                while True:
                    check_prompt = f"""
                                    Check for errors in the code that cause it to fail to execute as expected. 
                                    Please output an answer in pure JSON format according to the following schema. The JSON object must be parsable as-is. DO NOT OUTPUT ANYTHING OTHER THAN JSON, AND DO NOT DEVIATE FROM THIS SCHEMA:

                                        {{
                                            "code_wrong": {{
                                                "reason": string,
                                                "answer": boolean(true when need to fix the code bug)
                                            }}
                                        }}
                                    """.strip()
